...
Here you can see sample F2221 seems to be distant from all of the other samples indicating it is likely an outlier sample. 
...
In our case it looks like the cut height of 15 would cut F2221 and retain the rest of the samples. 
...
> **NOTE**: The WGCNA package has a built in function called ```adjacency()``` that will generate the pairwise similarity measurements and the adjacency matrix but before we call the function we need to identify what value we want to use for our threshold parameter.
...
We can then plot this data frame to better visualize what \beta value we should choose. 
...
Using the following data set find its appropriate soft threshold parameter.

```{r}
f <- curl('https://raw.githubusercontent.com/vfrench-397/AN588_RVignette/main/FemaleLiver-Data/LiverMale3600.csv')
...
Once our network is constructed, we can begin to extract some meaningful relationships. We can use hierarchical clustering yet again to cluster the network into **modules**.  

Lets quickly define module before we go on: A module is a group of gene profiles that are highly correlated, or have a high topological overlap. 
...
To identify modules from this gene dendrogram, we will use the ```cutreeDynamic()``` function. This will allow us to set a minimum cluster size. For genomic data like this it is more beneficial to set minimum module sizes relatively high as we are working with high loads of data. The authors of WGCNA recommend to start at a ```minClusterSize = 30```. 
...
we will need to add an argument ```use= "complete" ```. This command
...
```
This is the clustering dendrogram of samples based on their Euclidan distance. The white means a low value, red demonstrates a high value, and the grey areas are missing entries.

#### Quantifying Module Trait Associations

Identify modules that are significantly associated with external traits by correlating the trait with eigengenes (summary or representative gene for each module). Correlate eigengenes with external traits, and look for significant association.
...
There are many modules and traits. A graphical representation will be helpful in viewing the table. Each association value is color coded by correlation value.
...
```
Each row corresponds to a module eigengene, and the columns correspond to a trait. Each cell corresponds to a p-value and correlation. Those with strong positive correlations are shaded a darker red while those with stronger negative correlations become more blue.
...
Try this analysis the Glucose trait/module correlations from the heat map.

HINT: Define the variable "Glucose" containing the Glucose column

### **ANSWER 4**

```{r}
# Define the variable "Glucose" containing the Glucose column of datTrait
...
geneTraitSignificance = as.data.frame(cor(expression.data, Glucose, use = "p"))
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples))
names(geneTraitSignificance) = paste("GS.", names(Glucose), sep="")
names(GSPvalue) = paste("p.GS.", names(Glucose), sep="")
head(GSPvalue)
```
...
It is possible to study the relationship among found modules. Eigengenes can be used as representative profiles and quantify module similiarity by the correlation of eigengenes.

You are able to generate a summary plot of the eigengene network using the function "plotEigengeneNetworks." Using this function, you are able to isolate a clinical trait (or multiple traits), to visualize how the trait fits into the eigengene network. Much like in the Target Gene Identification section, you can focus on a clinical trait, like weight, except rather than displaying the relationship in a plot, you can display the relationship on a heatmap.
...
= 90)
```
This function above produces a dendrogram of eigengenes and isolated trait, as well as the corresponding heat map. You can separate the two using the following code: 
