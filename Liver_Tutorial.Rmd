...
You can create a a sample dendrogram and plot it above a sample-trait heatmap to visualize the relationship between traits and individual samples. #Vicky: I don't think this is necessary tbh

```{r}
sampleTree2 = hclust(dist(datTraits), method = "average")
traitColors = numbers2colors(datTraits, signed = FALSE) # Converts traits to a color representation: white means a low measured value of that trait, red means high, grey means missing entry
# Plot the sample dendrogram and the heatmap underneath
...
Once you have the gene signficance and the corresponding p-values you can create a module-trait heatmap to neatly visualize the module-trait relationships 
```{r}
...
Those with strong positive correlations are shaded a darker red while those with stronger negative correlations become more blue. The associated p value is also logged in parentheses underneath the gene significance. 
